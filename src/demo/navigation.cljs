...
(defonce ReactNavigationAddons (js/require "react-navigation-addons"))
(defonce Enhance (aget ReactNavigationAddons "enhance"))
...
;;;; Enhanced Navigators
(defonce EnhancedStackNavigator (Enhance StackNavigator))
...
(defn react-component?
  "Spec conforming function. Accepts a Reagent Component fn (React class) or returns :cljs.spec.alpha/invalid."
  [c]
  (cond
    (ric/react-class? c) c
...
(defn react-element?
  "Spec conforming function.  Accepts either a Reagent component fn (react element, conforms a react class to an element), or returns :cljs.spec.alpha/invalid."
  [e]
  (cond
    (isValidElement e) e
...
(defn fn-or-react-component?
  "Now *expects* a reagent component if a fn is supplied.
...
(s/def :react-navigation.navigationOptions/all
  (s/keys :opt-un [:react-navigation.navigationOptions/title
...
;;;; Enhanced Navigators for cljs
(defn enhanced-stack-navigator [routeConfigs stackNavigatorConfig]
  (let [routeConfigs-conformed (s/conform :react-navigation/RouteConfigs routeConfigs)
        StackNavigatorConfig-conformed (s/conform :react-navigation.StackNavigator/StackNavigatorConfig stackNavigatorConfig)]
    (assert (not= routeConfigs-conformed :cljs.spec.alpha/invalid))
    (assert (not= StackNavigatorConfig-conformed :cljs.spec.alpha/invalid))
    (if StackNavigatorConfig-conformed
      (EnhancedStackNavigator (clj->js routeConfigs-conformed) (clj->js StackNavigatorConfig-conformed))
      (EnhancedStackNavigator (clj->js routeConfigs-conformed)))))
